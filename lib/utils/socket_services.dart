...
class SocketServices {
  ...
  static Future<void> socketConnect() async {
    try {
      socket = io.io(
        Api.baseUrl,
        io.OptionBuilder()
            .setTransports(['websocket']).setQuery({"globalRoom": "globalRoom:${Database.loginUserId}"}).build(),
      );
  ...
  static Future<void> onReadMessage({
    ...
    if (socket != null && socket!.connected) {
      Utils.showLog(
          "Login User Id => ${Database.loginUserId} >>> Last Chat UserId => $lastVisitChatUserId >>> Message Sender User Id $senderUserId");
...
// >>>>> >>>>> >>>>> >>>>> Socket Listen Data Set Method <<<<< <<<<< <<<<< <<<<<
  ...
  static Future<void> onGetNewGift({required Map gift}) async {
    ...
    LiveUserSendGiftBottomSheetUi.giftType.value == 3
