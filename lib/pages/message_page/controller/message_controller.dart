...
class MessageController extends GetxController {
  ...
  Future<void> onGetMessageUsers() async {
    fetchMessageUserModel =
        await FetchMessageUserApi.callApi(loginUserId: Database.loginUserId);

    if (fetchMessageUserModel?.data != null &&
        (fetchMessageUserModel?.data?.isNotEmpty ?? false)) {
      final paginationData = fetchMessageUserModel?.data ?? [];

      Utils.showLog(
          "Message User => Page Index : ${FetchMessageUserApi.startPagination} Length : ${paginationData.length}");
  ...
  Future<void> onSearchMessageUser(String searchText) async {
    ...
    searchMessageUserModel = await SearchMessageUserApi.callApi(
        loginUserId: Database.loginUserId, searchText: searchText);
  ...
  void onCreateSearchHistory() {
    if (searchController.text.trim().isNotEmpty &&
        !searchMessageUserHistory.contains(searchController.text)) {
      searchMessageUserHistory.add(searchController.text);
      Database.onSetSearchMessageUserHistory(
          searchMessageUserHistory); // Add In Database
      update(["onChangeSearchHistory"]);
    }
  }
  ...
  void onDeleteSearchHistory(int index) {
    ...
    Database.onSetSearchMessageUserHistory(
        searchMessageUserHistory); // Reset Database
    update(["onChangeSearchHistory"]);
  }
}
