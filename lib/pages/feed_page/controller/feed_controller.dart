...
class FeedController extends GetxController {
  ...
  Future<void> onGetPost() async {
    fetchPostModel = null;
    // fetchPostModel = await FetchPostApi.callApi(
    //     loginUserId: Database.loginUserId, postId: BranchIoServices.eventId);
    fetchPostModel = await FetchPostApi.callApi(
        loginUserId: Database.loginUserId, postId: '');

    if (fetchPostModel?.post != null) {
      if (fetchPostModel!.post!.isNotEmpty) {
        mainPost.addAll(fetchPostModel?.post ?? []);
        update(["onGetPost"]);
      } else {
    ...
    await ImagePickerBottomSheetUi.show(
      ...
      onClickGallery: () async {
        ...
        if (images.isNotEmpty) {
          Get.toNamed(AppRoutes.uploadPostPage, arguments: {"images": images});
        }
      },
      onClickCamera: () async {
        Get.back();
