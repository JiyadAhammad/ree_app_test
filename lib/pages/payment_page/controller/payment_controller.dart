...
class PaymentController extends GetxController implements IAPCallback {
  ...
  final paymentMethodList = [
    ...
    {"icon": AppAsset.icPaypalLogo, "title": "Paypal", "size": "18.0"},
  ];
      ...
      try {
        ...
        RazorPayService().init(
          ...
          callback: () async {
            ...
            Get.dialog(const LoadingUi(), barrierDismissible: false); // Start Loading...
            final isSuccess = await CreateCoinPlanHistoryApi.callApi(
                loginUserId: Database.loginUserId, coinPlanId: coinPlanId, paymentType: "Razorpay");
            Get.back(); // Stop Loading...
            if (isSuccess) {
              Utils.showToast(EnumLocal.txtCoinRechargeSuccess.name.tr);
              Get.close(2);
      ...
      try {
        ...
        StripeService()
          ...
          callback: () async {
            ...
            Get.dialog(const LoadingUi(), barrierDismissible: false); // Start Loading...
            final isSuccess = await CreateCoinPlanHistoryApi.callApi(
                loginUserId: Database.loginUserId, coinPlanId: coinPlanId, paymentType: "Stripe");
            Get.back(); // Stop Loading...
            if (isSuccess) {
              Utils.showToast(EnumLocal.txtCoinRechargeSuccess.name.tr);
              Get.close(2);
